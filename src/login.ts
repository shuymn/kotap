 ...
 */
export const login = (profile: string): RTE.ReaderTaskEither<Page, Error, Page> => (
  page: Page
): TE.TaskEither<Error, Page> =>
  pipe(
...
const gotoLoginPage = (credential: Credential): RTE.ReaderTaskEither<Page, Error, Page> => (
  page: Page
): TE.TaskEither<Error, Page> =>
  pipe(
...
const tryToLogin = ({ id, password }: Credential): RTE.ReaderTaskEither<Page, Error, Page> => (
  page: Page
): TE.TaskEither<Error, Page> =>
  pipe(
