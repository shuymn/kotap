...
export const ERROR = {
  ...
  NO_CREDENTIALS_FILE: (fullpath: string): string =>
    `Could not read credentials file.\nPath: ${fullpath}`,
  INVALID_JSON: (fullpath: string): string =>
    `Invalid json file given.\nPath: ${fullpath}`,
  NO_CREDENTIAL_PROFILE: (profile: string): string =>
    `Profile: ${profile} is not found`,
  INVALID_CREDENTIAL: (profile: string): string =>
    `The schema of credential profile "${profile}" is invalid`,
  LAUNCH_BROWSER: "Failed to launch the browser",
  INITIALIZE_PAGE: "Failed to page initialization",
  GO_TO_LOGIN_PAGE: (url: string): string =>
    `Failed to go to the login page.\nURL: ${url}`,
  ...
  INVALID_DATE:
    "Invalid date given. The format of a date is yyyy-MMdd-HHmm (e.g. 2020-0204-0304)",
  INVALID_TYPE:
    'Invalid type given. Only both "request" and "recorder" are supported',
  ALREADY_DONE:
    "Both punching in and punching out on the specified date has already done",
  ...
  REQUEST_RECORDING: (reason: string): string =>
    `Failed to request to record.\nReason: ${reason}`,
};
...
export const XPATH = {
  ...
  APPROVED_PUNCHING_IN:
    "//select[@id='recording_type_code']/option[@selected and contains(text(), '出勤')]",
  APPROVED_PUNCHING_OUT:
    "//select[@id='recording_type_code']/option[@selected and contains(text(), '退勤')]",
